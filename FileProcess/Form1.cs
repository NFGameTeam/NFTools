    ...
    public partial class Form1 : Skin_Metro
    {
        ...
        bool CreateStructXML(string strFile, string strFileName)
        {
            try
            {
                ...
                // 打开excel
                Workbook book = new Workbook(strCurPath + "/" + strFile);
                ...
                foreach (Worksheet sheet in book.Worksheets)
                {
                    ...
                    int nRowCount = sheet.Cells.MaxDataRow;
                    int nColCount = sheet.Cells.MaxDataColumn;

                    switch (strSheetName.ToLower())
                    {
                        case "property":
                            {
                                for (Int32 row = 1; row <= nRowCount; ++row)
                                {
                                    string testValue = sheet.Cells[row, 0].StringValue;
                                    ...
                                    for (Int32 col = 0; col <= nColCount; ++col)
                                    {
                                        try
                                        {
                                            string name = sheet.Cells[0, col].StringValue;
                                            string value = sheet.Cells[row, col].StringValue;
                                            Cell valueCell = sheet.Cells[row, col];
                                            if (valueCell.Type == CellValueType.IsBool)
                                            {
                                                value = (valueCell.BoolValue ? "1" : "0");
                                            }
                                ...
                                for (Int32 row = 1; row <= nRowCount; ++row)
                                {
                                    ...
                                    for (Int32 col = 0; col < nColCount; ++col)
                                    {
                                        try
                                        {
                                            ...
                                            componentNode.SetAttribute(name, value);
                                        }
                                        catch (Exception err)
                                        {
                                            MessageBox.Show(strFile + "\n" + "Sheet: " + strSheetName + "\n" + "Cell: " + row.ToString() + ", " + col.ToString() + "\n" + "This cell is empty or error\n" + err.Message);
                        ...
                        default:
                            {
                                ...
                                for (Int32 col = 0; col < nRecordStart; ++col)
                                {
                                    ...
                                    {
                                        string name = sheet.Cells[0, col].StringValue;
                                        string value = sheet.Cells[1, col].StringValue;
                                        Cell valueCell = sheet.Cells[1, col];
                                        if (valueCell.Type == CellValueType.IsBool)
                                        {
                                            value = (valueCell.BoolValue ? "1" : "0");
                                        }
